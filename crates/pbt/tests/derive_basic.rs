...
use gnrs_test_pbt::{run, Arbitrary};
...
fn derive_failure_shrinks_through_fields() {
    use gnrs_test_pbt::{forall_with, Config, Outcome};
    let cfg = Config {
        cases: 100,
        seed: 999,
        max_shrinks: 256,
        ...
        shrink_mode: gnrs_test_pbt::ShrinkMode::Greedy,
    };
...
fn attribute_macro_zero_args() {
    gnrs_test_pbt::prop_assert_eq!(1 + 1, 2);
}

#[gnrs_test_pbt::pbt]
fn attribute_macro_single_arg(n: u8) {
    gnrs_test_pbt::prop_assert_eq!(n.wrapping_add(0), n);
}

#[gnrs_test_pbt::pbt]
fn attribute_macro_two_args(a: i32, b: i32) {
    gnrs_test_pbt::prop_assert_eq!(a.wrapping_add(b), b.wrapping_add(a));
}

#[gnrs_test_pbt::pbt]
fn attribute_macro_with_derive(s: NamedStruct) {
    // プロパティが自明になるケースをスキップするために prop_assume!を使います。
    gnrs_test_pbt::prop_assume!(!s.c.is_empty());
    gnrs_test_pbt::prop_assert_eq!(s.c.len(), s.c.iter().count());
}

#[gnrs_test_pbt::pbt(cases = 50, max_size = 20)]
